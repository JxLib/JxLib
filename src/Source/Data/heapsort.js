...
Jx.Sort.Heapsort = new Class({
    ...
    sort: function(){
	    ...
	    if (count > 2) {
	        ...
	        if ((this.comparator((this.data[0]).get(this.col),(this.data[1]).get(this.col)) > 0)) {
	            this.data.swap(0,1);
	        }
	    }
    ...
    siftDown: function(start, end){
        ...
        while (root*2 <= end) {
            var child = root *2;
            if ((child + 1 < end)&& (this.comparator((this.data[child]).get(this.col),(this.data[child+1]).get(this.col)) < 0)) {
                child = child + 1;
            }
            if ((this.comparator((this.data[root]).get(this.col),(this.data[child]).get(this.col)) < 0)){
